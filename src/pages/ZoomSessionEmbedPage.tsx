...
export default function ZoomSessionEmbedPage() {
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-[var(--color-bg)] p-6 text-pulse-800 dark:text-[var(--text-primary)]">
      ...
      {!data.zoomConnected ? (
        ...
        >
          <p className="text-pulse-600 dark:text-[var(--text-secondary)] text-center max-w-md">
            Session <strong className="text-pulse-900 dark:text-[var(--text-primary)]">{data.sessionCode}</strong> is linked.
            Present from Qesto launchpad; participants join via code while you host in Zoom.
          </p>
          <p className="mt-4 text-xs text-pulse-500 dark:text-[var(--text-muted)]">Code: {data.sessionCode}</p>
        </section>
      )}
    </main>
  )
