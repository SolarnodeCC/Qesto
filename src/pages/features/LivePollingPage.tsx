...
const latencyRows = [
  { hop: 'Hop 01', title: 'Client → edge POP.', desc: 'TLS to the nearest Cloudflare edge. Participant never talks to an origin.', lat: '~22ms', sub: 'illustrative p50' },
  { hop: 'Hop 02', title: 'Edge → Durable Object.', desc: 'Session state lives in a single DO, pinned to the colo closest to first-mover.', lat: '~8ms', sub: 'illustrative intra-colo' },
  { hop: 'Hop 03', title: 'DO → WebSocket fan-out.', desc: 'Tally refresh broadcast to every connected participant over persistent WS.', lat: '~12ms', sub: 'illustrative fanout' },
  { hop: 'Hop 04', title: 'Render.', desc: 'Bar moves on the projected screen. Same pipe, no client poll.', lat: '~16ms', sub: 'illustrative paint' },
  { hop: 'Total', title: 'Tap to bar movement.', desc: 'End-to-end benchmark pending Cloudflare-backed staging validation.', lat: 'TBD', sub: 'validation pending', highlight: true },
]
...
export default function LivePollingPage() {
  return (
    <MainLayout>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link to="/features/ai-insights" className={btnSecondary + ' text-base px-7 py-3.5 dark:bg-[var(--color-surface-elevated)] dark:border-[var(--color-border-strong)] dark:text-[var(--text-primary)]'}>
                  See AI Insights
                </Link>
              </div>
            </div>
            ...
            {/* Poll preview */}
            <div className="bg-white dark:bg-[var(--color-surface)] rounded-xl p-7" style={shadowElevated}>
              <div className="flex justify-between mb-3.5 text-[11px] font-bold uppercase tracking-widest text-pulse-500 dark:text-[var(--text-muted)]">
              ...
              <h3 className="font-bold text-[22px] tracking-tight mb-4 text-pulse-900 dark:text-[var(--text-primary)]" style={displayFont}>
              ...
              {[
                ...
                <div key={lbl} className="flex items-center gap-2.5 mb-2 text-sm">
                  <span className="w-32 font-medium text-pulse-700 dark:text-[var(--text-secondary)]">{lbl}</span>
      ...
      {/* Question types */}
      <Reveal as="section" className="py-16 bg-pulse-50 dark:bg-[var(--color-bg-subtle)]">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {questionTypes.map(({ icon, title, desc, tag }) => (
              <div
                key={title}
                className="bg-white dark:bg-[var(--color-surface)] rounded-xl p-5.5 grid gap-4 items-start"
              ...
              >
                ...
                <div>
                  <h3 className="font-semibold text-[16px] mb-1.5 text-pulse-900 dark:text-[var(--text-primary)]">{title}</h3>
                  <p className="text-[13.5px] text-pulse-500 dark:text-[var(--text-muted)] leading-relaxed mb-1.5">{desc}</p>
                  <span className="text-[11px] text-pulse-500 dark:text-[var(--text-muted)]" style={monoFont}>{tag}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Latency ladder */}
      <Reveal as="section" className="py-16 bg-white dark:bg-[var(--color-bg)]">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="rounded-2xl overflow-hidden border border-pulse-200 dark:border-[var(--color-border)] divide-y divide-pulse-200 dark:divide-[#1E2A45]">
            {latencyRows.map(({ hop, title, desc, lat, sub, highlight }) => (
              <div
                key={hop}
                className="bg-white dark:bg-[var(--color-surface)] px-6 py-4.5 grid items-center gap-6 text-sm"
              ...
              >
                ...
                <span className="text-pulse-800 dark:text-[var(--text-secondary)]">
                  <strong className="font-semibold">{title}</strong> {desc}
                </span>
                <div className="text-right" style={monoFont}>
                  <div
                    className={`text-[13px] font-bold ${highlight ? 'text-teal-700 dark:text-teal-400' : 'text-pulse-800 dark:text-[var(--text-secondary)]'}`}
                  >
                    {lat}
                  </div>
                  <div className="text-[11px] text-pulse-500 dark:text-[var(--text-muted)] uppercase tracking-widest">{sub}</div>
                </div>
              </div>
            ))}
          </div>
