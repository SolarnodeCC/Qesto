  ...
  return (
    <MainLayout>
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <div className="text-xs font-bold tracking-widest uppercase text-teal-700 mb-3">Qesto for HR</div>
            ...
            {/* Mode card */}
            <div className="bg-white rounded-[20px] p-6" style={shadowElevated}>
              {/* Tabs */}
              <div className="flex gap-1.5 p-1 bg-pulse-100 rounded-xl mb-5">
                  ...
                  ([key, label]) => (
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-lg text-[13px] font-semibold transition-all ${
                        mode === key
                          ? 'bg-white text-pulse-900 shadow-[var(--shadow-card)]'
                          : 'text-pulse-500'
                      }`}
                    >
                      {label}
              ...
              <div className="space-y-2.5 mb-3">
                {[1, 2].map((i) => (
                  <div key={i} className="flex items-center gap-3 p-3.5 rounded-xl bg-pulse-50">
                    <div
                    ...
                    <div className="flex-1">
                      <div className="text-sm font-semibold text-pulse-900">
                        {mode === 'ident' ? (i === 1 ? 'Alex Kim' : 'Priya Rao') : 'Participant'}
                      </div>
                      <div className="text-[13px] text-pulse-500">
              ...
              {/* Tally */}
              <div className="p-4 rounded-xl bg-pulse-50 space-y-2">
                {[
                  ...
                  <div key={lbl} className="flex items-center gap-2.5 text-[13px]">
                    <span className="w-[90px] font-medium text-pulse-700">{lbl}</span>
      ...
      <section className="py-16 bg-pulse-50">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-xs font-bold tracking-widest uppercase text-teal-700 mb-3">Proof</div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {[
              ...
              <div key={label} className="bg-white rounded-2xl p-6" style={shadowCard}>
                <div className="text-[13px] text-pulse-500 mb-2">{label}</div>
                <div className="font-bold text-4xl tracking-tight text-pulse-900 mb-2" style={displayFont}>{value}</div>
                <div className="text-[13px] text-pulse-500 leading-snug">{note}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-xs font-bold tracking-widest uppercase text-teal-700 mb-3">Where it fits</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {useCases.map(({ tag, title, desc, items }) => (
              <div key={title} className="bg-white rounded-2xl p-7" style={shadowCard}>
                <div className="text-[11px] font-bold uppercase tracking-widest text-teal-700 mb-2">{tag}</div>
                <h3 className="font-semibold text-[20px] mb-2.5 text-pulse-900">{title}</h3>
                <p className="text-pulse-500 text-sm leading-relaxed mb-4">{desc}</p>
                <ul className="list-disc pl-4 space-y-1 text-sm text-pulse-500">
                  {items.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <blockquote className="max-w-3xl mx-auto text-center">
            ...
            <footer className="text-sm text-pulse-500">
              <strong className="text-pulse-900">Priya Ramanathan</strong> · Chief People Officer, mid-market SaaS
              (pilot customer)
            </footer>
          </blockquote>
        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-xs font-bold tracking-widest uppercase text-teal-700 mb-3">Compliance posture</div>
          <h2 className="font-bold text-4xl tracking-tight mb-8 text-pulse-900" style={displayFont}>
