...
import { StatusBadge as CanonicalStatusBadge } from '../../ui/StatusBadge'

// Session card, its status badge, and loading skeleton — extracted from Dashboard.tsx (R-05).
...
export function StatusBadge({ status }: { status: string }) {
  ...
  const key = STATUS_KEY[status as SessionStatus]
  if (!key) {
    return (
    ...
    >
      ...
      <div ref={menuRef} className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
        <button
          ...
          className="flex items-center justify-center w-7 h-7 rounded-md bg-white/80 dark:bg-[var(--color-surface)]/80 backdrop-blur-sm text-pulse-600 dark:text-[var(--text-secondary)] hover:bg-white dark:hover:bg-[#1C2540] border border-pulse-200 dark:border-[var(--color-border)] shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 transition-colors"
        >
