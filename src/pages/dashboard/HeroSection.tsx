  ...
  return (
    <section aria-labelledby="hero-heading">
      <h1
        id="hero-heading"
        tabIndex={-1}
        className="font-display text-4xl lg:text-5xl font-bold text-pulse-900 dark:text-[var(--text-primary)] focus:outline-none"
      >
        {t('greeting', { name: userName })}
      </h1>
      <p className="mt-2 text-lg text-pulse-500 dark:text-[var(--text-secondary)]">
        {t('sessionSubtext')}
      </p>
      <div className="mt-6 flex flex-wrap gap-3">
        {townhallFeatureEnabled ? (
              ...
              >
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-semibold text-pulse-900 dark:text-[var(--text-primary)]">{t('interactiveSession')}</span>
                    <span className="block text-xs text-pulse-500 dark:text-[var(--text-secondary)]">{t('interactiveSessionDesc')}</span>
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-pulse-900 dark:text-[var(--text-primary)]">{t('townhallSession')}</span>
                      {!isTeamPlan && (
                    ...
                    <span className="block text-xs text-pulse-500 dark:text-[var(--text-secondary)]">{t('townhallSessionDesc')}</span>
                  </span>
                </button>
              </div>
            )}
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-pulse-900 dark:bg-[var(--text-primary)] text-white dark:text-pulse-900 px-5 py-2.5 text-sm font-semibold shadow-card hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 transition-opacity"
          >
            <Sparkles size={15} aria-hidden="true" />
            {t('newSession').replace('+ ', '')}
          </button>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-pulse-300 dark:border-[var(--color-border-strong)] bg-white dark:bg-transparent text-pulse-700 dark:text-[var(--text-secondary)] px-5 py-2.5 text-sm font-semibold hover:border-teal-400 dark:hover:border-teal-600 hover:text-teal-700 dark:hover:text-teal-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 transition-colors"
        >
          {t('startFromTemplate')}
        </button>
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-pulse-200 dark:border-[var(--color-border)] bg-white dark:bg-transparent text-pulse-500 dark:text-[var(--text-muted)] px-5 py-2.5 text-sm font-semibold cursor-not-allowed opacity-60"
        >
          {t('importSession')}
        </button>
      </div>
