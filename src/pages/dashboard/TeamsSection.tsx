...
export function TeamsSection({ teams, teamsLoading }: TeamsSectionProps) {
  ...
  return (
    <section id="section-teams" aria-labelledby="teams-heading">
      <h2 id="teams-heading" className="text-xl font-semibold text-pulse-900 dark:text-[var(--text-primary)] mb-4">
        {t('teams')}
      </h2>
      {teamsLoading ? (
        ...
        <p className="text-sm text-pulse-500 dark:text-[var(--text-secondary)]">{t('noTeamsYet')}</p>
      ) : (
        <ul className="divide-y divide-pulse-200 dark:divide-[var(--color-border)] rounded-xl border border-pulse-200 dark:border-[var(--color-border)]">
          {teams.map((team) => (
            <li key={team.id} className="flex items-center justify-between gap-4 p-4">
              <div>
                ...
                <p className="text-xs text-pulse-500 dark:text-[var(--text-muted)] mt-0.5 capitalize">{team.plan} plan</p>
              </div>
              <Link
