...
import { Cog, HelpCircle, Home, Lightbulb, Library, Menu, Moon, Settings, Sparkles, Sun, Users } from 'lucide-react'
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[var(--color-bg)]">
      ...
      <nav
        ...
        className={[
          'fixed inset-y-0 left-0 z-30 flex w-[240px] flex-col border-r border-[color:var(--color-border)]',
          'bg-[var(--color-surface)] dark:bg-[var(--color-bg-subtle)] transition-transform duration-200 ease-[cubic-bezier(0.2,0,0,1)]',
          'lg:static lg:z-auto lg:translate-x-0',
          sidebarOpen ? 'translate-x-0' : '-translate-x-full',
        ].join(' ')}
      >
        {/* Logo */}
        <div className="flex h-14 shrink-0 items-center px-6 border-b border-[color:var(--color-border)]">
          <Link
            to="/"
          ...
          >
            <Sparkles size={14} aria-hidden="true" className="text-violet-500 shrink-0" />
            Qesto
          </Link>
        </div>
        ...
        <ul className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5" role="list">
                ...
                <button
                  ...
                  className={[
                    ...
                    isActive
                      ...
                      : 'text-[var(--text-secondary)] hover:bg-pulse-100 dark:hover:bg-white/5 hover:text-[var(--text-primary)]',
                  ].join(' ')}
                >
          ...
          {isSuperuser && (
            <li>
              ...
              >
                <Cog size={18} aria-hidden="true" className="shrink-0" />
                Admin
              </Link>
            </li>
          )}
        ...
        {/* Account links */}
        <div className="border-t border-[color:var(--color-border)] px-2 py-3 space-y-0.5">
          <Link
            to="/settings"
            onClick={() => setSidebarOpen(false)}
            ...
            className={[
              ...
              onSettingsPage
                ...
                : 'text-[var(--text-muted)] hover:bg-pulse-100 dark:hover:bg-white/5 hover:text-[var(--text-secondary)]',
            ].join(' ')}
          >
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <header className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-[color:var(--color-border)] bg-[var(--color-surface)] dark:bg-[var(--color-bg-subtle)] px-4 lg:px-8">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <form
              ...
              className="hidden sm:flex items-center gap-0 rounded-lg border border-[color:var(--color-border-strong)] overflow-hidden"
              aria-label={t('joinSessionPrompt')}
            >
              <span className="pl-3 pr-1 text-xs text-[var(--text-muted)] whitespace-nowrap select-none">
                {t('joinSessionPrompt')}
              </span>
              <input
                type="text"
              ...
              <button
                ...
                className="px-2.5 py-1.5 text-sm text-[var(--text-muted)] hover:text-teal-600 dark:hover:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-500/10 disabled:opacity-0 transition-colors border-l border-[color:var(--color-border-strong)]"
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center justify-center w-12 h-12 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-pulse-100 dark:hover:bg-white/8 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 transition-colors"
            >
              {scheme === 'dark' ? (
            ...
            {auth.status === 'authenticated' && (
              <div ref={avatarRef} className="relative">
                ...
                {avatarMenuOpen && (
                  <div
                    role="menu"
                    className="absolute right-0 top-full mt-1 z-[var(--z-dropdown)] min-w-[180px] rounded-xl border border-[color:var(--color-border)] bg-[var(--color-surface-elevated)] shadow-elevated py-1 animate-page-enter"
                  >
                    <div className="px-3 py-2 text-xs text-pulse-500 text-[var(--text-muted)] border-b border-[color:var(--color-border)]">
                      {userEmail}
                    </div>
                    <button
                      type="button"
                      ...
                      className="w-full text-left px-3 py-2 text-sm text-[var(--text-secondary)] hover:bg-pulse-50 dark:hover:bg-white/5 hover:text-red-600 dark:hover:text-red-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal-500 transition-colors"
                    >
                      {tCommon('logout')}
                    </button>
                  </div>
