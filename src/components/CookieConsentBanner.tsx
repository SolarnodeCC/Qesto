...
export function CookieConsentBanner() {
  ...
  return (
    <div
      role="region"
      aria-label={t('aria')}
      className="fixed inset-x-0 bottom-0 z-50 border-t border-pulse-200 dark:border-[#2A3858] bg-surface dark:bg-[#1C2540] shadow-elevated animate-page-enter"
    >
      <div className="mx-auto flex max-w-5xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-body-s text-pulse-700 dark:text-[#A8B3CC]">
          <p className="font-semibold text-ink dark:text-[#F0F2F8]">{t('title')}</p>
          <p className="mt-1">
            {t('description')}{' '}
            <a
              href="/privacy"
