...
export default function QuestionList({
  ...
  return (
    <section
      ...
      className="rounded-xl border border-[var(--surface-border)] dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-surface)] shadow-card overflow-hidden"
    >
      {/* Card header */}
      <div className="flex items-center justify-between gap-3 px-6 py-3.5 border-b border-[var(--surface-border)] dark:border-[var(--color-border)]">
      ...
      {orderedQuestions.length === 0 && (
        <div className="px-6 py-4 space-y-4">
          <p className="text-sm text-[var(--text-muted)] dark:text-[var(--text-muted)]">{t('no_questions_hint')}</p>
          <div className="rounded-xl border border-violet-100 dark:border-violet-900/40 bg-violet-50/60 dark:bg-violet-900/10 p-4 space-y-3">
            ...
            <form onSubmit={(e) => void handleAIGenerate(e)} className="space-y-2">
              ...
              <Button type="submit" disabled={aiGenerating} size="sm" className="bg-violet-600 hover:bg-violet-700 focus-visible:ring-violet-500">
                {aiGenerating
      ...
      {/* Question rows */}
      {orderedQuestions.length > 0 && (
        <ul className="divide-y divide-[var(--surface-border)] dark:divide-[var(--color-border)]">
              ...
              {editingId === q.id ? (
                <div className="px-6 py-4 space-y-3">
                  <div className="space-y-1">
                    <label htmlFor={`edit-prompt-${q.id}`} className="text-xs font-medium text-[var(--text-muted)]">{t('edit_prompt_label')}</label>
                  ...
                  <div className="space-y-1">
                    <label htmlFor={`edit-kind-${q.id}`} className="text-xs font-medium text-[var(--text-muted)]">{t('edit_kind_label')}</label>
                  ...
                  <div className="flex gap-2">
                    <Button type="button" onClick={() => void saveEdit()} disabled={editSaving || editPrompt.trim().length === 0} size="sm">
                      {editSaving ? '…' : t('save_question')}
                    </Button>
                    <Button type="button" variant="secondary" onClick={cancelEdit} disabled={editSaving} size="sm">
                      {t('cancel_edit')}
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 px-[18px] py-[13px] cursor-grab active:cursor-grabbing">
                  ...
                  <span className="font-mono text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)] w-4 shrink-0">{index + 1}</span>
                  <span className={`shrink-0 px-2 py-0.5 rounded-full text-[11px] font-semibold capitalize ${KIND_BADGE[q.kind] ?? KIND_BADGE.poll}`}>
                    {q.kind}
                  </span>
                  <span className="flex-1 text-sm text-[var(--text-primary)] dark:text-[var(--text-primary)] truncate">{q.prompt}</span>
      ...
      {addingQuestion && (
        <div className="px-6 py-4 border-t border-teal-100 dark:border-teal-900/40 bg-teal-50/30 dark:bg-teal-500/5 space-y-3">
          <div className="space-y-1">
            <label htmlFor="add-prompt" className="text-xs font-medium text-[var(--text-muted)]">{t('edit_prompt_label')}</label>
            <textarea id="add-prompt" value={addPrompt} onChange={(e) => setAddPrompt(e.target.value)} rows={2} autoFocus
              className="w-full rounded-lg border border-pulse-300 dark:border-[var(--color-border-strong)] dark:bg-[var(--color-surface-elevated)] dark:text-[var(--text-primary)] px-3 py-2 text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-500" />
          </div>
          <div className="space-y-1">
            <label htmlFor="add-kind" className="text-xs font-medium text-[var(--text-muted)]">{t('edit_kind_label')}</label>
          ...
          <div className="flex gap-2">
            <Button type="button" onClick={() => void handleAddQuestion()} disabled={addSaving || addPrompt.trim().length === 0} size="sm">
              {addSaving ? '…' : t('save_question')}
            </Button>
            <Button type="button" variant="secondary" onClick={() => { setAddingQuestion(false); setAddPrompt(''); setAddError(null) }} disabled={addSaving} size="sm">
