  ...
  return (
    <section className="space-y-3" aria-label="Energizers">
      ...
      {energizers.map((energizer) => (
        <div
          key={energizer.id}
          className="flex items-center gap-4 rounded-xl border border-[var(--color-border,var(--color-border))] dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-surface)] px-4 py-3.5 shadow-card"
        >
          {/* Violet icon well */}
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-[var(--text-muted,var(--text-muted))] dark:text-[var(--text-muted)] mt-0.5 truncate">
