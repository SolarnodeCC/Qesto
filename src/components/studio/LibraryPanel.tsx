  ...
  return (
    <section aria-labelledby="studio-library-heading" className="rounded-lg border border-pulse-200 bg-white p-5 dark:border-[var(--color-border-strong)] dark:bg-pulse-900/40">
      ...
      {!loading && items.length === 0 && (
        <p className="mt-3 text-sm text-pulse-500 dark:text-[var(--text-muted)]">{t('library.empty')}</p>
      )}
          ...
          {items.map((item) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-pulse-500 dark:text-[var(--text-muted)]">{t('library.useCount', { count: item.use_count })}</p>
              </div>
