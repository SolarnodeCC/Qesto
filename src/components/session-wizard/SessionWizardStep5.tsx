...
export function SessionWizardStep5({
  ...
  return (
    <div className="space-y-4">
      <p className="caption text-pulse-500">{t('step5.consent_text')}</p>
      ...
      <section className="rounded-xl border border-pulse-200 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-surface)] p-3 space-y-1">
        ...
        <p className="caption text-pulse-500">{goal}</p>
        {templateSeedName && (
          <p className="caption font-medium text-teal-700 dark:text-teal-300">
      ...
      <section className="rounded-xl border border-pulse-200 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-surface)] p-3 space-y-2">
        ...
        {activeQuestions.length === 0 ? (
          <p className="caption text-amber-600">{t('step5.no_questions')}</p>
        ) : (
          <ul className="space-y-1.5">
            {activeQuestions.map((q) => (
              <li key={q.id} className="text-sm flex items-start gap-2">
                ...
                <span className="ml-auto caption text-pulse-500 whitespace-nowrap">{kindLabel(q.kind)}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="rounded-xl border border-pulse-200 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-surface)] p-3 space-y-1">
        ...
        {energizerId ? (
          ...
          <p className="caption text-pulse-500">{t('step5.no_energizer')}</p>
        )}
      </section>
