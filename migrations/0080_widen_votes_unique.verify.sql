...
-- option_id) key (expect 1), that every production votes index survived the
-- rebuild (expect 1 each), and that the temporary rebuild table was cleaned up
-- (expect 0).
...
SELECT COUNT(*) AS idx_votes_session_submitted_plain FROM sqlite_master
