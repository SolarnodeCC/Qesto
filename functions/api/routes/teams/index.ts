...
export function mountTeamRoutes(parent: Hono<{ Bindings: Env; Variables: Vars }>) {
  ...
  app.delete('/:id', async (c) => {
    ...
    // (audit #12). These vectors exist only to power team-scoped cross-session
    // intelligence — the very feature being torn down — and they carry session
    // titles as metadata in a cross-tenant index. The sessions and their D1
    // rows are deliberately left intact; only the derived embeddings go, and
    // they are regenerated by a future insights run if the team is recreated.
    if (c.env.DECISIONS_VECTORIZE) {
      try {
        const { results } = await c.env.DB.prepare(
          `SELECT id FROM sessions WHERE team_id = ?1`,
        )
          .bind(team.id)
          .all<{ id: string }>()
        const ids = (results ?? []).map((r) => r.id)
        for (let i = 0; i < ids.length; i += 100) {
          await c.env.DECISIONS_VECTORIZE.deleteByIds(ids.slice(i, i + 100))
        }
      } catch (vecErr) {
        // Best-effort: a vector purge failure must not block the team delete.
        logEvent({ event: 'team.delete.vectorize_skip', teamId: team.id, error: String(vecErr) })
      }
    }
