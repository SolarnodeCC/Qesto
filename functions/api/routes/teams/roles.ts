// CODE-SPLIT — team custom roles + role assignments (no behavior change).
//
// Routes:
//   GET    /:id/roles                                  list roles + assignments
//   POST   /:id/roles                                  create custom role
//   PATCH  /:id/roles/:roleId                          update custom role
//   DELETE /:id/roles/:roleId                          delete role + assignments
//   POST   /:id/roles/:roleId/assignments             assign role to member
//   DELETE /:id/roles/:roleId/assignments/:userId     unassign role
import { ulid } from '../../lib/ulid'
import { recordAuditEvent } from '../../lib/audit'
import { validatePermissions } from '../../lib/authz'
import { CreateCustomRoleSchema, PatchCustomRoleSchema, AssignRoleSchema } from './schemas'
import { loadTeam, findMember, isMemberOrOwner } from './store'
import { requireTeamPermission, roleDto } from './permissions'
import type { CustomRoleRow, RoleAssignmentRow, TeamsApp } from './types'

const ROLE_SELECT =
  `SELECT id, team_id, name, permissions_json, created_by, created_at, updated_at FROM custom_roles WHERE id = ?1 AND team_id = ?2`

export function mountTeamRoleRoutes(app: TeamsApp) {
  // ───────────────────────────────────────────────────────────────────────────
