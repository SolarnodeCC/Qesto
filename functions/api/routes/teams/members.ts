// CODE-SPLIT — team membership invites + removal (no behavior change).
//
// Routes:
//   POST   /:id/members          invite member by email (issues magic-link token)
//   DELETE /:id/members/:userId  remove member (owner/admin; owner protected)
...
import { loadTeam, saveTeam, removeUserTeam } from './store'
import { requireTeamPermission } from './permissions'
import type { TeamsApp } from './types'

export function mountTeamMemberRoutes(app: TeamsApp) {
  // ───────────────────────────────────────────────────────────────────────────
