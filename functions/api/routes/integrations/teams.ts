// TEAMS-01: Microsoft Teams integration routes (mirrors the Slack endpoints).
//
// Endpoints (all under /api/integrations/teams):
//   GET    /connect      — redirect to Microsoft OAuth (PKCE)
//   GET    /callback     — exchange code, persist token
//   GET    /status       — returns { connected, channelName?, groupName? }
//   POST   /disconnect   — delete token + config
//   POST   /config       — host picks the delivery channel
