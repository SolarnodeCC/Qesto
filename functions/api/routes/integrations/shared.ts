// Shared building blocks for the Slack/Teams/Zoom/Salesforce/Notion integration
// route modules (ADR-0008 IntegrationProvider pattern). Config shapes, KV key
// helpers, the HMAC-signed OAuth state token, the provider factory, and the
// small cross-provider helpers all live here so each provider module stays
// focused on its own endpoints.
//
// State token: a compact base64(JSON) blob signed with HMAC-SHA256 over
// JWT_SECRET. We carry `{ teamId, userId, exp }` and verify on the callback,
// avoiding the JWT library to keep the payload minimal.
...
export function getTeamsProvider(
...
export async function resolvePrimaryTeamId(env: Pick<Env, 'TEAMS_KV'>, userId: string): Promise<string | null> {
