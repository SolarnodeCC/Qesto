...
export function mountObservabilityRoutes(
  ...
  app.get('/observability/snapshot', authMiddleware, adminMiddleware, async (c) => {
    ...
    const thresholds = await loadThresholds(c.env)
    const degraded: string[] = []

    // Single grouped scan of metrics_summary; partitioned in JS into HTTP
    // routes, D1 spans (route LIKE 'd1.%'), and AI spans (route LIKE 'ai.%').
    let rows: RouteAgg[] = []
    try {
    ...
    if (c.env.METRICS_KV) {
      try {
        const agg = await aggregateLiveMetrics(c.env.METRICS_KV, 5)
    ...
    const named: Array<{ name: string; idx: { describe?: () => Promise<unknown> } | undefined }> = [
      { name: 'DECISIONS_VECTORIZE', idx: c.env.DECISIONS_VECTORIZE as never },
      { name: 'HELP_VECTORIZE', idx: c.env.HELP_VECTORIZE as never },
    ]
    ...
    const snapshot: ObservabilitySnapshot = {
      ...
      components: {
        workers: {
          ...
          note: '4xx/5xx split not instrumented — error_count is the combined total',
        },
        d1: {
          spans: d1Spans,
