...
describe('Sprint 24 contract', () => {
  ...
  it('keeps custom role management wired into team settings', () => {
    // TeamSettings.tsx orchestrates role API calls; UI strings live in CustomRolesSection
    expect(teamSettingsSource).toContain('/roles')
    expect(customRolesSource).toContain('team:manage_members')
    expect(customRolesSource).toContain('Assign role')
    expect(customRolesSource).toContain('Unassign')
  })
