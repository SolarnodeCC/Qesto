...
describe('CONNECT-INVITE-01 — mint + verify round-trip', () => {
  it('mints a verifiable targeted invite', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, {
      sid: 'sess-1',
      invitee: 'guest-team',
      scope: 'participate',
      now: 1000,
    ...
    const verified = await verifyFederationInvite(SECRET, minted.token, { now: 2000 })
  ...
  it('defaults invitee to null (open invite) and scope to participate', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 'sess-2', now: 1000 })
    expect(minted.ok).toBe(true)
    if (!minted.ok) return
...
describe('CONNECT-INVITE-01 — sovereign exclusion at mint (ADR-0059)', () => {
  it('refuses to mint for a sovereign host — returns a violation, not a token', async () => {
    const r = await mintFederationInvite(SECRET, { teamId: 'gov', isSovereign: true }, { sid: 's' })
...
describe('CONNECT-INVITE-01 — verification failures', () => {
  it('rejects a tampered payload (bad signature)', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 'sess-3', now: 1000 })
    if (!minted.ok) throw new Error('mint failed')
    const tampered = 'x' + minted.token.slice(1)
    const r = await verifyFederationInvite(SECRET, tampered, { now: 2000 })
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.reason).toBe('bad_signature')
  })

  it('rejects the wrong secret', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 'sess-4', now: 1000 })
    if (!minted.ok) throw new Error('mint failed')
  ...
  it('rejects an expired invite', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 'sess-5', ttl: 100, now: 1000 })
    if (!minted.ok) throw new Error('mint failed')
    const r = await verifyFederationInvite(SECRET, minted.token, { now: 1000 + 101 })
    expect(r.ok).toBe(false)
  ...
  it('rejects malformed tokens', async () => {
    for (const bad of ['', 'no-dot', '.', 'abc.']) {
      const r = await verifyFederationInvite(SECRET, bad)
      expect(r.ok).toBe(false)
    }
  })
})
...
describe('CONNECT-INVITE-01 — tenant admission', () => {
  it('open invite admits any tenant', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 's', invitee: null, now: 1 })
    if (!minted.ok) throw new Error('mint failed')
    expect(inviteAdmitsTenant(minted.claims, 'anyone')).toBe(true)
  })

  it('targeted invite admits only the named tenant', async () => {
    const minted = await mintFederationInvite(SECRET, HOST, { sid: 's', invitee: 'guest', now: 1 })
    if (!minted.ok) throw new Error('mint failed')
