...
describe('migration safety metadata (HLT-021/HLT-030)', () => {
  ...
  it('0081 restores every votes index for environments that already applied broken 0080', () => {
    const sql = readFileSync(join(MIGRATIONS, '0081_restore_votes_indexes.sql'), 'utf8')
    const verify = readFileSync(join(MIGRATIONS, '0081_restore_votes_indexes.verify.sql'), 'utf8')

    for (const indexName of VOTES_INDEXES) {
      expect(sql, `0081 should recreate ${indexName}`).toContain(`CREATE INDEX IF NOT EXISTS ${indexName}`)
      expect(verify, `0081 verify should assert ${indexName}`).toContain(`'${indexName}'`)
    }
  })
