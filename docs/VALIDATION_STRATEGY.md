...
### Routes (HLT-031)
The following files have unchecked casts in request/response processing:
- `functions/api/routes/teams.ts` — Team data parsing from DB
- `functions/api/routes/templates.ts` — Template JSON parsing
- `functions/api/routes/knowledge-base.ts` — KB data parsing
- `functions/api/routes/insights.ts` — Insights response parsing
- `functions/api/routes/billing.ts` — Stripe webhook parsing
- `functions/api/routes/admin.ts` — Admin data parsing (also has SQL issues)
- `functions/api/routes/auth/register-analyze.ts` — Analysis request parsing

### Libraries (HLT-031)
- `functions/api/lib/audit.ts` — Audit event KV parsing
- `functions/api/lib/authz.ts` — Authorization rule parsing
- `functions/api/lib/oauth.ts` — OAuth state/token parsing
...
- `functions/api/lib/insights-*.ts` — Insights data structures
