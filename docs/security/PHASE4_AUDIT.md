...
| A02 | Cryptographic failures | JWT HS256 with `JWT_SECRET` from Pages secret; 14-day TTL; HttpOnly + SameSite=Lax cookie. Magic-link OTT stored as SHA-256 hash in KV. | — | `functions/api/routes/auth.ts`, `functions/api/lib/jwt.ts` | **OK** — secret is Pages secret, never in `wrangler.toml`. |
