...
- [~] CSP headers set (present, but `script-src 'unsafe-inline'` weakens it)
- [x] Webhook signatures verified (Stripe HMAC + constant-time; outbound HMAC-signed)
- [x] D1 queries parameterized
- [ ] **Access-control layer effective end-to-end** — blocked by HIGH #1 (RBAC no-op) and HIGH #2 (LDAP IDOR)
