...
_2026-11-03 (UTC) — **Sprint 99 shipped: v7.0 GA — Engagement Intelligence Network.** Final sprint of the S85–S99 arc. `ADR-0063` accepted (v7.0 platform certification + v6.x deprecation policy). v7.0 GA release engineering: platform `RELEASES` += `7.0.0` (GA, sprint 99); `/api/platform/version` api → `7.0.0`; `/api/platform/certification` → certifiedVersion `7.0.0`, `pentest6: complete`, `isolationProof: verified` (ADR-0062 / `QA-CONNECT-SCALE-01`), `deprecationPolicy` → ADR-0063 (AAA stays bounded `partial`); new `/api/platform/v6-sunset` (currentGa `7.0.0`, v6 maintenance ends `2029-11-03`). XR beta completion (feature-flagged `beta-xr`): `FE-XR-LAUNCHER-01` real WebXR device detection (`navigator.xr` immersive-vr/-ar probe — Quest 3 / iOS Safari 16+ / Android Chrome; `src/xr/useWebXrSupport.ts`) + `XR-FALLBACK-01` explicit 2D graceful-degrade path (`fallback_notice` surfaced for non-WebXR; voting never gated) + i18n in 5 locales. GA epics: REACTIONS, PULSE, COPILOT, LEARN, SOVEREIGN+, CONNECT, STUDIO; XR beta only. Gates: tsc clean · Vitest 2200 · build green · check:i18n green · no eval-gate trigger (feature-flagged + data-only). See [`SPRINT99_EXECUTION.md`](../releases/SPRINT99_EXECUTION.md), [`v7.0.0.md`](../releases/v7.0.0.md), [`ADR-0063`](../../adr/ADR-0063-v7-platform-certification.md). Arc closed at v7.0 GA; next horizon is S100+/v7.1 (XR GA path, deferred backlog)._
...
_2026-06-17 — **PR554 perf follow-ups shipped (PERF)**: (1) trimmed the eager critical bundle — 13 non-home page modules + the authenticated-only HelpChatWidget are now `React.lazy` in `src/App.tsx`, leaving only `Home` eager on the `/` LCP path; (2) HTML-shell short-TTL/SWR edge caching (`public/_headers` `/*` → `public, max-age=0, s-maxage=60, stale-while-revalidate=86400`) to cut the ~1.4s TTFB. Devops call resolved: safe vs. the deploy commit-parity check, no pipeline change. See [`ADR-0065`](../../adr/ADR-0065-html-shell-swr-edge-caching.md)._
