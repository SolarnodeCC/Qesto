...
This folder is a portable design system for Qesto: brand foundations, UI tokens, icons, logos, and high-fidelity UI kit recreations of the public site and the host dashboard. Use it to prototype or produce anything Qesto-branded.

---

## Sources

All visual, copy, and token material was extracted from the attached repository (no Figma provided):

- **GitHub:** `SolarnodeCC/Qesto` @ `main` (private)
- **Key files referenced:**
  - `docs/specs/design-tokens.json` — canonical token source (v1.0.0, 2026-04-19)
  - `docs/specs/WEBSITE_DESIGN_SPEC.md` — 51KB target-state spec
  - `docs/BRAND_VOICE.md` — tone, vocabulary, CTA rules
  - `docs/DESIGN_GRID_GUIDE.md` — 12/8/4 responsive grid, 4px baseline
...
**CDN.** Lucide is used via the package in the codebase. For standalone HTML artifacts in this design system, link via `<script src="https://unpkg.com/lucide@latest"></script>` then `<i data-lucide="sparkles"></i>` + `lucide.createIcons()`. This is a **CDN substitution** of the bundled package — the glyphs are identical.
...
- `README.md` — this file.
- `SKILL.md` — Claude Code skill entry point (cross-compatible with Agent Skills).
- `colors_and_type.css` — CSS custom properties for the full token set + semantic type roles. Drop in via `@import` or `<link>`.
- `assets/` — brand marks (`favicon.svg`, `icon-192.png`, `icon-512.png`).
- `preview/` — ~20 small HTML cards that populate the Design System tab (type, colors, spacing, components, brand).
- `ui_kits/website/` — marketing-site UI kit: hero, nav, pricing card, feature strip, footer. Interactive index.html.
- `ui_kits/dashboard/` — host-facing UI kit: dashboard sessions list, AI badge, metric card, Session Launchpad, wizard step shells. Interactive index.html.
- `ui_kits/present/` — projected 1920×1080 live-polling surface (bars, join panel, QR, consent footer).
- `ui_kits/participant/` — 4-screen mobile join flow: join code · consent round · rank vote · confirmation.
...
1. **Fonts — Google Fonts only.** The repo references Inter, Syne, JetBrains Mono, all loaded from Google Fonts. No self-hosted woff2s were shipped in the repo; `colors_and_type.css` imports from `fonts.googleapis.com`. If you want offline/CSP-safe bundles, ask and I'll swap to self-hosted.
2. **Icons — CDN swap.** The codebase uses `lucide-react`; standalone HTML artifacts in this system load lucide via unpkg. Same glyphs; different delivery.
3. **No bespoke imagery.** The repo has no product screenshots, illustrations, or marketing photography checked in. Preview cards and UI kits use neutral placeholders (skeleton blocks, lucide icons). Ship real screenshots or illustrations and I'll wire them in.
4. **No Figma access.** All visual decisions come from the codebase + spec documents. If a Figma file exists, reattach it for pixel-level refinement.
