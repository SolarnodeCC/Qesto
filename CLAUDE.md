...
10. **Design token conventions (ADR-0071):** cards and panels use `rounded-xl` (24px via `--radius-xl`); buttons use `rounded-lg` (16px via `--radius-lg`). Prefer CSS custom-property tokens (`var(--text-primary)`, `var(--color-border)`, etc.) over hardcoded hex. See [`ADR-0071`](./knowledge-base/adr/ADR-0071-design-system-v1.md) and [`SPEC_DESIGN_SYSTEM_OVERVIEW`](./knowledge-base/specifications/domain/SPEC_DESIGN_SYSTEM_OVERVIEW.md).
11. **Plan reads go through `effectivePlan` (ADR-0074):** while the temporary free-access window can be open, `users.plan` (and a team's plan in `TEAMS_KV`) is the *stored* tier, not the served one. Anything gating a feature reads `c.get('plan')`, which `planMiddleware` has already resolved. Code that reads a plan **outside a request** — background jobs, cron, cached payloads — must pass it through `effectivePlan(env, stored)` from [`lib/free-access.ts`](./functions/api/lib/free-access.ts) or it silently misses the window. Only billing/analytics that must report what someone actually holds read the stored tier (`c.get('plan_stored')`). See [`ADR-0074`](./knowledge-base/adr/ADR-0074-temporary-free-access-promo.md) for the bypass register.
...
/seo-audit      → loads seo-audit.md skill pack       (claude-seo orchestrator: /seo audit, parallel specialists, SEO Health Score — Cursor + Cloud Agents)
...
| Medium | **sonnet** | `qesto-frontend`, `qesto-devops`, `qesto-analytics`, `qesto-sales`, `qesto-knowledge`, `qesto-seo-reviewer`, `qesto-seo-audit`, `qesto-seo-technical`, `qesto-seo-content`, `qesto-seo-schema`, `qesto-seo-sitemap`, `qesto-seo-geo`, `qesto-seo-competitor-pages` | React + Tailwind components, client WebSocket state, `wrangler.toml` env matrix, CI workflows, Analytics Engine queries, deal qualification + objection strategy, KB integrity + requirement traceability, organic-visibility / technical-SEO audits, claude-seo parallel dispatch |
